...
client.on("guildMemberAdd", member => {  
  const kanal = member.guild.channels.cache.find(r => r.id === "KANAL ID"); //HOŞGELDİN MESAJI ATILACAĞI KANAL IDSINI GİRİN ÖRNEĞİN UNREGİSTER CHATİ
  ...
  moment.locale("tr");
  kanal.send(":tada: Sunucumuza Hoş Geldin ! <@" + member + "> \n\n Hesabın "+ gecen +" Önce Oluşturulmuş "+kontrol+" \n\n Sunucu kurallarımız <#RULESKANALIDSI> kanalında belirtilmiştir. Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek. \n\n Seninle beraber **" + member.guild.memberCount + "** kişi olduk , Tagımızı alarak bizlere destek olabilirsin , Kayıt olmak için teyit odalarına girip ses teyit vermen gerekiyor <@&YETKILIROLID> seninle ilgilenecektir  İyi eğlenceler !")
  });
//--------------------------------TAG ROL KISMI ENDLESS-------------------------------\\
client.on("userUpdate", async (oldUser, newUser) => {
if (oldUser.username !== newUser.username) {
let endlesstag = "TAGINIZ"; //Tagınızı yazın
let endlessunucu = "SUNUCU ID'sı"; //Sunucu ID'sini giriniz
let endlesskanal = "LOG KANAL ID" //Mesajın atılacağı log kanalını giriniz
let endlessrol = "TAGLIROLU ID";//Taglı rolünün ID'sini giriniz 
